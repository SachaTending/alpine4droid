...
echo --- begin of alpine4droid setup log --- | tee setup_log.log > /dev/null

log () {
    echo [*] $@
    echo Alpine4droid: [*] $@ | tee -a setup_log.log > /dev/null
}
...
setup_base () {
    ...
    log Extracting...
    cat tmp/target.tar.gz | pv | tar xz -C $1

    # Write resolv.conf and install openrc
    log Settuping base system...
    echo nameserver 1.1.1.1 | tee $1/etc/resolv.conf > /dev/null
    run_in_chroot $1 /sbin/apk add alpine-base openssh --progress -q
    echo PermitRootLogin yes | tee -a $1/etc/ssh/sshd_config > /dev/null
...
log Creating dirs..
mkdir $instpath/system $instpath/apex $instpath/data

log Creating integrate.sh\(starts container\)
tee $instpath/integrate.sh <<EOF > /dev/null
...
tee $instpath/run.sh <<EOF > /dev/null
# Generated by alpine4droid
...
chmod +x $instpath/integrate.sh
chmod +x $instpath/run.sh
...
tee /data/adb/service.d/alpine4droid_$installpath.sh <<EOF > /dev/null
# Generated by alpine4droid

$instpath/integrate.sh

EOF

chmod +x /data/adb/service.d/alpine4droid_$installpath.sh
...
echo --- end of alpine4droid setup log --- | tee -a setup_log.log > /dev/null
